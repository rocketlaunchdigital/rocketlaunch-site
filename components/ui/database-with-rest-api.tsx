...
const DatabaseWithRestApi = ({
  ...
  const id = useId();
  const mask1Id = `db-mask-1-${id}`;
  const mask2Id = `db-mask-2-${id}`;
  const mask3Id = `db-mask-3-${id}`;
  const mask4Id = `db-mask-4-${id}`;
  const gradId = `db-purple-grad-${id}`;
      ...
      >
        ...
        <defs>
          ...
          <radialGradient id={gradId} fx="1">
            <stop offset="0%" stopColor={lightColor || "#8F00FF"} />
